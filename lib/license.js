...
module.exports = function(args, str) {
    var match, version;
    try {
    ...
    } else if (args.customLicenses) {
        console.log(args.customLicenses);
        for (let e of args.customLicenses.split(",")) {
            const customLic = e.split(":");

            if (customLic.length !== 2) {
                continue;
            }

            const isMatch = str.match(customLic[1]);

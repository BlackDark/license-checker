...
var flatten = function(options) {
    ...
    /*istanbul ignore else*/
      ...
      (licenseData.type || licenseData.name)
            ...
            moduleInfo.licenses = license(
                options._args,
                licenseData.type || licenseData.name
            );
        } else if (typeof licenseData === "string") {
            moduleInfo.licenses = license(options._args, licenseData);
        }
    } else if (license(options._args, json.readme)) {
        moduleInfo.licenses = license(options._args, json.readme);
    }
    ...
    files.forEach(function(filename, index) {
        ...
        moduleInfo.licenses.indexOf("Custom:") === 0
                ...
                moduleInfo.licenses = license(options._args, content);
            }
